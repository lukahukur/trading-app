...
const ChartIntervals = ()=>{
    ...
    let intervalArrayFull:Time[] = ['1s','1m','3m','5m','15m','30m','1h','2h','4h','6h','8h','12h','1d','3d','1w','1M'];
    ...
    let intervalArraySmHTML = intervalArraySm.map((e,i) => <button key={i} 
                                                            style={{color:clr(e)}} 
                                                            onClick={()=>clickHandler(e)}
                                                            >{e}</button>);

    let intervalArrayFullHTML = intervalArrayFull.map(e => <button key={e} 
                                                            style={{color:clr(e)}} 
                                                            onClick={()=>clickHandler(e)}
                                                            >{e}</button>);                      
    useEffect(()=>{
                ...
                <div className={styles.btn_wrapper}>
                    <span className='items-center flex justify-around w-96'>
                        ...
                        <span  className={styles.d_menu}>
                            ...
                            <span style={{display:'flex',alignItems:'center',justifyContent:'spaceBetween',width:'45px'}}>
                                <span ref={IRef} className='text-sm' style={{color:'#26C6DA'}}/>
                                <DropSvg color={'gray'}/>
                            </span>
